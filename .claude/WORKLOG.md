...
### 12 · CV specialist review + split detection experiments for Adarsh
**Agenda:** 1) Independent production-CV review of `docs/ARCHITECTURE.md`, flagged for Devesh 2) Turn the detection-recall risk into two parallel, non-colliding experiments for Adarsh + Devesh 3) Push

- ✅ 1 Full review delivered in chat (not committed as a file — Devesh has it). Headline finding: the **3×3 tile grid probably doesn't catch the back-row faces it was built to catch** — SCRFD's internal ~640px resize still shrinks a 90px face to ~26px inside a 2000px+ tile, below the doc's own 50px floor. Also flagged: agglomerative clustering at n=32k likely won't hit the "5 second rerun" the build order assumes without `fastcluster` or blocking; the Tier-1 selfie match uses one fixed threshold across wildly different cluster sizes (2,104-face bride vs 4-face guest) with no margin/ratio test, right where lookalike relatives are the known risk; and biometric templates get generated for every photographed guest at Step 3 with no consent mechanism beyond the photographer's own agreement. · `—` · `verified`
- ✅ 2 Wrote `docs/FACE-DETECTION-EXPERIMENTS.md` proposing two independent fixes to the tiling problem, addressed to Adarsh: **Idea A** — calibrate tile size from the detector's actual resize factor instead of a fixed 3×3 grid; **Idea B** — coarse-to-fine multi-scale detection instead of a fixed grid at all. Created `experiments/tiled-grid-detection/` and `experiments/multiscale-pyramid-detection/` (README stub each, no code yet) so the two ideas can be worked in parallel without touching each other's files — reuses the existing `.claude/ACTIVE-WORK.md` claim protocol for whoever picks which. Shared success metric: detection recall by face-height bucket, real data only (cricket set is explicitly called out as insufficient for this). · `docs/FACE-DETECTION-EXPERIMENTS.md`, `experiments/tiled-grid-detection/README.md`, `experiments/multiscale-pyramid-detection/README.md` · `local`
